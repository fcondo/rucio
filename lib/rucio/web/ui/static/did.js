...
show_replicas = function(replicas, scope, name) {
    var table_files = [];
    var dav_replicas = [];
...
handle_dataset = function(scope, name) {
    ...
    $('#load_replicas').click(async function() {
        var replicas = await r.list_replicas({'scope': scope, 'name': name})
        show_replicas(replicas)
    });
};

handle_file = async function(scope, name) {
    $('#result').append(html_replicas_base);
    $('#result').append(html_parents_base);

    var replicas = await get_replicas(scope, name)
    show_replicas(replicas, scope, name)
    rses = []
    replicas.forEach(function(replica) {
        Object.keys(replica['rses']).forEach(function(rse){
            rses.push(rse)
        })
    })
    load_parent_dids(scope, name);
    var rules = get_rules(scope, name).catch(function() {return []})
    var did_informations = get_did_information(scope, name).catch(function() {return []})
    var requests = get_requests(scope, name, rses).catch(function() {return []})
    Promise.all([rules, did_informations, requests]).then(function(data) {
        var tableData = []
        var transferData = []
        var lifetimeData = []
        data.forEach(function(result) {
            result.forEach(function(point) {
                if(point['transferData']) {
                    transferData.push(point['transferData'])
                }
                lifetimeData.push(point['lifetimeData'])
                tableData.push(point['tableData'])
            })
        })
        show_did_tracking_table(tableData)
        var timelineData = [{'label': 'Transfers', 'data': transferData}, {'label': 'Lifetime', 'data': lifetimeData}]
        console.log(timelineData)
        show_did_tracking_timeline(timelineData)
    })
};

get_replicas = function(scope, name) {
    return new Promise(function(resolve, reject) {
        r.list_replicas({'scope': scope, 'name': name, 'success': function(replicas) {
            resolve(replicas)
        }, 'error': function(jqXHR, textStatus, errorThrown) {
            reject(errorThrown)
        }})
...
get_rules = function(scope, name) {
    // Load rule information
    return new Promise(function(resolve, reject) {
        r.did_get_rules({'scope': scope, 'name': name, 'success': function(rules) {
            var data = []
            rules.forEach(function(rule) {
                data.push({
                    lifetimeData: {
                        type: TimelineChart.TYPE.POINT,
                        at: new Date(rule['created_at']),
                        desc: 'Created at: ' + new Date(rule.created_at).toLocaleString() + '<br>RSE Exp: ' + rule.rse_expression
                    },
                    tableData: {
                        'timestamp': new Date(rule.created_at), 'detail': 'Rule ' + rule.id + ' created<br>RSE expression: ' + rule.rse_expression
                    }
                })
...
get_requests = function(scope, name, rses) {
    // Load transfer request information
    return new Promise(function(resolve, reject) {
        promises = []
        rses.forEach(function(rse) {
            promises.push(new Promise(function(resolve, reject) {
                r.get_request_by_did({'rse': rse, 'scope': scope, 'name': name, 'success': function(request) {
                    if(request['request_type'] == 'TRANSFER') {
                        data = []
                        data.push({
                            lifetimeData: {
                                type: TimelineChart.TYPE.POINT,
                                at: new Date(request['created_at']),
                                desc: 'Created at: ' + new Date(request.created_at).toLocaleString()
                            },
                            tableData: {
                                timestamp: new Date(request.created_at),
                                'detail': 'Transfer request created<br>Destination RSE: ' + request.dest_rse_id
                            }
                        })
                        data.push({
                            lifetimeData: {
                                type: TimelineChart.TYPE.POINT,
                                at: new Date(request['submitted_at']),
                                desc: 'Submitted at: ' + new Date(request.submitted_at).toLocaleString()
                            },
                            tableData: {
                                'timestamp': new Date(request.submitted_at), 'detail': 'Transfer submitted'}
                        })
                        data.push({
                            lifetimeData: {
                                type: TimelineChart.TYPE.POINT,
                                at: new Date(request['transfered_at']),
                                desc: 'Finished at: ' + new Date(request.transfered_at).toLocaleString()
                            },
                            tableData: {
                                timestamp: new Date(request.transfered_at),
                        ...
                        data.push({
                            transferData: {
                                ...
                                from: new Date(request['started_at']),
                                to: new Date(request['transfered_at']),
                            }
                        })
                        resolve(data)
                    }
...
get_did_information = function(scope, name) {
    // Load DID information
    return new Promise(function(resolve, reject) {
        data = []
        r.get_did({'scope': scope, 'name': name, 'success': function(did) {
            if (did.expired_at) {
                data.push({
                    lifetimeData: {
                        type: TimelineChart.TYPE.POINT,
                        at: new Date(did['expired_at']),
                        desc: 'Expired at: ' + new Date(did.expired_at).toLocalString()
                    },
                    tableData: {
                        timestamp: new Date(rule.expired_at),
                        detail: 'Expired'
                    }
                })
            }
...
show_did_tracking_timeline = function(timelineData) {
    const element = document.getElementById('chart');
    const timeline = new TimelineChart(element, timelineData, {
        enableLiveTimer: true,
        tip: function(d) {
            return d.desc || `${d.from}<br>${d.to}`;
        }
    })
}

show_did_tracking_table = function(tableData) {
    for(var i=0; i < tableData.length; i++) {
        $('#tracking_table').append($('<tr><th>' + tableData[i]['timestamp'].toLocaleString() + '</th><td>' + tableData[i]['detail'] + '</td></tr>'));
    }
}

load_did_metadata = function(scope, name) {
    r.did_get_metadata({
        'scope': scope,
        'name': name,
        success: function(data) {
...
load_did_generic_metadata = function(scope, name) {
...
build_page = function() {
    ...
    load_did_metadata(scope, name);
    load_did_generic_metadata(scope, name);
};
...
$(document).ready(function(){
    ...
    if ((storage.isSet('expiration') && Date.now() - storage.get('expiration') > 3600000) || !storage.isSet('expiration')) {
        ...
        r.list_rses({
            expression: 'browser_enabled=True&availability_read=True',
        ...
        r.list_rses({
            expression: 'browser_enabled=True&availability_read=True&availability_write=True&type=SCRATCHDISK',
            success: function(rses) {
                scratch_rses = [];
                for(i = 0; i < rses.length; i++){
