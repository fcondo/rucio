...
def import_accounts(accounts, session=None):
    ...
    for account in missing_accounts:
        ...
        email = account['email']
        account_module.add_account(account=account_name, type=AccountType.USER, email=email, session=session)
        identities = account.get('identities', [])
        if identities:
            import_identities(identities, account_name, old_identities, old_identity_account, email, session=session)
    ...
    for account in outdated_accounts:
        account_name = account['account']
        email = account['email']
        old_account = old_accounts[account_name]
        if email and old_account['email'] != email:
            account_module.update_account(account_name, key='email', value=email, session=session)
